...
class ResNet54(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, class_drop=0.5):
    ...
    def forward(self, input, mixup_lambda=None, mcd=False):
        """
        ...
        x = F.dropout(x, p=0.2, training=self.training or mcd, inplace=True)
        x = self.resnet(x)
        x = F.avg_pool2d(x, kernel_size=(2, 2))
        x = F.dropout(x, p=0.2, training=self.training or mcd, inplace=True)
        ...
        x = x1 + x2
        x = F.dropout(x, p=self.class_drop, training=self.training or mcd)
        x = F.relu_(self.fc1(x))

        clipwise_output = torch.sigmoid(self.fc_audioset(x))

        embedding = F.dropout(x, p=self.class_drop, training=self.training or mcd)
