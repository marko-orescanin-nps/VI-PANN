''' 
Copied from https://github.com/qiuqiangkong/audioset_tagging_cnn/blob/master/pytorch/pytorch_utils.py
...
def forward(model, generator, return_input=False, return_target=False, n_samples=1, mcd=False):
    ...
    n_samples_output_dict = []
    for i in range(n_samples):
        output_dict = {}

        # Forward data to a model in mini-batches
        for _, batch_data_dict in enumerate(generator):
            batch_waveform = move_data_to_device(batch_data_dict['waveform'], device)
            
            with torch.no_grad():
                model.eval()
                batch_output = model(batch_waveform, mcd=mcd)
        ...
        if n_samples == 1:
            ...
            if i == 0:
                n_samples_output_dict = output_dict
            else:
                n_samples_output_dict['target'] = np.concatenate([n_samples_output_dict['target'], output_dict['target']], axis=1)
                n_samples_output_dict['clipwise_output'] = np.concatenate([n_samples_output_dict['clipwise_output'], output_dict['clipwise_output']], axis=1)

    return n_samples_output_dict
